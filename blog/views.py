...
def page_not_found_view(request, exception, template_name='blog/error_page.html'):
    ...
    return render(request, template_name,
                  {'message': '您访问的地址 ' + url + ' 是一个未知的地方.', 'statuscode': '404'}, status=404)


def server_error_view(request, template_name='blog/error_page.html'):
    return render(request, template_name,
                  {'message': '出错了，我已经收集到了错误信息，之后会抓紧抢修.', 'statuscode': '500'}, status=500)
...
def permission_denied_view(request, exception, template_name='blog/error_page.html'):
    if exception:
        logger.error(exception)
    return render(request, template_name,
                  {'message': '您没有权限访问此页面.', 'statuscode': '403'}, status=403)
